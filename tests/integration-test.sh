...
if [[ $DRONE_BRANCH == "master" || $DRONE_BRANCH == *"refs/tags"* || $DRONE_BRANCH == "releases/"* ]] && [[ $DRONE_REPO == "vmware/vic" ]] && [[ $DRONE_BUILD_EVENT == "push" || $DRONE_BUILD_EVENT == "tag" ]]; then
	...
	pabot --processes 9 --removekeywords TAG:secret --exclude skip tests/test-cases
    ...
    pabot --processes 9 --removekeywords TAG:secret --suite $testsuite --suite 7-01-Regression tests/test-cases
else
        echo "Running regressions"
    pabot --processes 9 --removekeywords TAG:secret --exclude skip --include regression tests/test-cases
fi
